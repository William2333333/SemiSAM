### Code From https://github.com/rstrudel/segmenter
### Source : https://arxiv.org/abs/2105.05633
...
class MTSegmenter(nn.Module):
    ...
    def forward(self, im):
        ...
        masks1 = self.decoder1(x, (H, W))

        masks1 = F.interpolate(masks1, size=(H, W), mode="bilinear", align_corners=False)
        masks1 = unpadding(masks1, (H_ori, W_ori))
        
        masks2 = self.decoder2(x, (H, W))

        masks2 = F.interpolate(masks2, size=(H, W), mode="bilinear", align_corners=False)
...
def MTcreate_segmenter(model_cfg):
    model_cfg = model_cfg.copy()
    decoder_cfg = model_cfg.pop("decoder")
    decoder_cfg["n_cls"] = model_cfg["n_cls"]

    encoder = create_vit(model_cfg)
    decoder1 = create_decoder(encoder, decoder_cfg)
    decoder2 = create_decoder(encoder, decoder_cfg)
    model = MTSegmenter(encoder, decoder1, decoder2, n_cls=model_cfg["n_cls"])

    return model


def load_model_segmenter(model_path, pretrained, nb_channel, nb_class):
    with open(model_path + "variant.yml", "r") as f:
        variant = yaml.load(f, Loader=yaml.FullLoader)
    net_kwargs = variant["net_kwargs"]

    model = create_segmenter(net_kwargs)
    if pretrained:
      data = torch.load(model_path + "segmenter.pth", map_location='cuda')
      checkpoint = data["model"]
      model.load_state_dict(checkpoint, strict=True)

    model.encoder.patch_embed.proj = torch.nn.Conv2d(nb_channel, 768, kernel_size=(8, 8), stride=(8, 8))
    model.decoder.head = nn.Linear(768, nb_class, bias=True)

    return model

def MTload_model_segmenter(model_path, pretrained, nb_channel, nb_class):
    with open(model_path + "variant.yml", "r") as f:
        variant = yaml.load(f, Loader=yaml.FullLoader)
    net_kwargs = variant["net_kwargs"]

    model = MTcreate_segmenter(net_kwargs)
    if pretrained:
      data = torch.load(model_path + "segmenter.pth", map_location='cuda')
      checkpoint = data["model"]
      model.load_state_dict(checkpoint, strict=True)
    ...
    model.encoder.patch_embed.proj = torch.nn.Conv2d(nb_channel, 768, kernel_size=(8, 8), stride=(8, 8))
    model.decoder1.head = nn.Linear(768, nb_class, bias=True)
    model.decoder2.head = nn.Linear(768, nb_class, bias=True)
