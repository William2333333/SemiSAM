...
from manage.manage_genkyst import genksyt_split, create_genkyst_dataset_3O
from datasets.dataset_genkyst import dataset_genkyst_3O
import argparse
import logging
import sys
import os
...
from utils.train_utils import launch_training_3O, dice_history
...
def train_genkyst(net_id, 
    ...
    create_genkyst_dataset_3O(output, train_ids, train_series, 'train', modality, size)
    
    create_genkyst_dataset_3O(output, val_ids, val_series, 'val', modality, size) 

    train_dataset = dataset_genkyst_3O(output, 'train', modality, vgg)

    val_dataset = dataset_genkyst_3O(output, 'val', modality, vgg)    
    ...
    return launch_training_3O(epochs, net, train_loader, val_loader, n_train, device, net_id, criterion, optimizer, output)
...
def get_args():
    ...
    parser.add_argument('-l', '--learning', type=int, default=10, dest='lr') # 20 # 2000
